...
export default function editorialPolicy(ctx) {
  ...
  ${prose(
    ...
    <ul>
      ...
      <li><strong>No prediction.</strong> The index summarizes activity already reported. It is not a forecast and not a live case count. Surveillance carries a lag of roughly one to two weeks, so FluTrack leads with a direction rather than a single figure.</li>
      ...
      <li><strong>No invented figures.</strong> No number, level or map colour may appear that is not derived from a reported value. Where a state reported nothing, the page says so.</li>
    </ul>
    ...
    <p>A snapshot file ships with the site so a page is legible immediately. It declares itself sample data in its own contents, and a live refresh may replace that label with a live-CDC-data badge only once it has produced usable signals for at least 25 of the 51 jurisdictions, with a valid week-ending date. That floor exists because a successful response carrying no usable rows once let sample data be badged as live — an error recorded in the <a href="/changelog/">changelog</a>.</p>

    <h2>Licensing: an editorial rule enforced in code</h2>
    <p>Some widely cited wastewater data — the WastewaterSCAN network, also referenced as SCAN or Verily — is published under a CC BY-NC 4.0 non-commercial licence. FluTrack carries advertising and affiliate links, which makes it a commercial use, so that data is excluded. The exclusion is not left to intention: the ingestion code runs a provenance filter that drops any wastewater row whose source field names those networks, and it has unit tests.</p>
    ...
    <p>FluTrack is free, funded by display advertising and disclosed affiliate links. Three rules keep that revenue away from the figures, set out at <a href="/affiliate-disclosure/">affiliate and advertising disclosure</a>. A commercial link may only be rendered by one shared component, which emits the disclosure immediately before the link, in fixed wording the caller cannot supply, with a sponsored and nofollow relationship on the anchor. A state's reading is never sold, sponsored or adjusted, and the threat level is not available as an advertising targeting parameter; that page also rules out targeting on precise location, alert status, symptom or condition, pregnancy, age, or inferred vulnerability. And every third party that touches visitor data is named by legal entity at <a href="/vendors/">vendors</a> — with its purpose, lawful basis, data categories, retention and deletion route — rendered from a single processor register that the <a href="/privacy/">privacy policy</a> reads from as well, so the two documents cannot describe a vendor differently.</p>
    ...
    <table>
      ...
      <tbody>
        ...
        <tr><td>The share image does not appear to encode per-state severity</td><td>The QA step decodes the image and fails if its map area contains severity-ramp colours, since no real per-state data exists at build time</td></tr>
        ...
        <tr><td>Every page has a title, description and canonical, and no internal link is dead</td><td>The QA step crawls the output and fails on a missing tag, a dead link, or a contact address at a domain that can never receive mail</td></tr>
