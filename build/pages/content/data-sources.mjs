...
export default function dataSources(ctx) {
  ...
  ${pageHeader({
    ...
    lede:
      "FluTrack is assembled entirely from the CDC's own respiratory surveillance — the same open, public-domain feeds anyone can download. This page documents each dataset behind the threat level, the licence it carries, and why we leave some sources out on purpose.",
  })}
  ...
  ${prose(`
    ...
    <ul>
      <li><strong>A bundled snapshot</strong> ships with the site and paints immediately. It is clearly labelled as illustrative sample data until a live refresh succeeds.</li>
      <li><strong>A live refresh</strong> then queries the CDC's public-domain Socrata (SODA) endpoints on <a href="https://data.cdc.gov/">data.cdc.gov</a> directly from your browser. When it succeeds, the snapshot is replaced and the provenance badge flips from <span class="badge badge--cached"><span class="badge__dot"></span>Cached</span> to <span class="badge badge--live"><span class="badge__dot"></span>Live</span>.</li>
    </ul>
  ...
  return {
    title: 'Data sources',
    description:
      "Every CDC dataset behind FluTrack's respiratory threat level — what each measures, its licence and cadence — and why we exclude non-commercial data.",
    path: '/data-sources/',
    body,
    changefreq: 'monthly',
    priority: 0.5,
